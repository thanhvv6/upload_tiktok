/**
 * PostgreSQL persistence helpers for profile groups (no HTTP layer).
 */

function httpError(status, message) {
  const err = new Error(message);
  err.status = status;
  return err;
}

export function initGroupSchema(db) {
  // Schema is now managed by migration files — no-op at runtime
}

function normalizeGroupName(name) {
  if (typeof name !== 'string') {
    throw httpError(400, 'Group name is required');
  }
  const trimmed = name.trim();
  if (trimmed.length === 0) {
    throw httpError(400, 'Group name is required');
  }
  return trimmed;
}

export async function getGroupById(db, id) {
  return db.prepare('SELECT id, name, created_at FROM groups WHERE id = ?').get(id);
}

export async function assertGroupExists(db, id) {
  if (!(await getGroupById(db, id))) {
    throw httpError(404, 'Group not found');
  }
}

export async function listGroups(db) {
  const rows = await db
    .prepare(
      `SELECT
          g.id,
          g.name,
          g.created_at,
          COUNT(p.id) AS profile_count
       FROM groups g
       LEFT JOIN profiles p ON p.group_id = g.id
       GROUP BY g.id, g.name, g.created_at
       ORDER BY g.created_at DESC`
    )
    .all();
  return rows.map((row) => ({
    ...row,
    profile_count: Number(row.profile_count),
  }));
}

export async function createGroup(db, { id, name }) {
  if (!id || typeof id !== 'string' || id.trim() === '') {
    throw httpError(400, 'Group id is required');
  }
  const trimmedName = normalizeGroupName(name);
  const existingName = await db
    .prepare('SELECT id FROM groups WHERE LOWER(name) = LOWER(?)')
    .get(trimmedName);
  if (existingName) {
    throw httpError(400, 'A group with this name already exists');
  }
  try {
    await db
      .prepare('INSERT INTO groups (id, name) VALUES (?, ?)')
      .run(id.trim(), trimmedName);
  } catch (e) {
    if (e && e.code === '23505') {
      throw httpError(400, 'A group with this name already exists');
    }
    throw e;
  }
}

export async function renameGroup(db, { id, name }) {
  await assertGroupExists(db, id);
  const trimmedName = normalizeGroupName(name);
  const conflict = await db
    .prepare('SELECT id FROM groups WHERE LOWER(name) = LOWER(?) AND id != ?')
    .get(trimmedName, id);
  if (conflict) {
    throw httpError(400, 'A group with this name already exists');
  }
  await db.prepare('UPDATE groups SET name = ? WHERE id = ?').run(trimmedName, id);
}

export async function deleteGroup(db, id) {
  await assertGroupExists(db, id);
  const row = await db
    .prepare('SELECT COUNT(*) AS n FROM profiles WHERE group_id = ?')
    .get(id);
  const n = Number(row.n);
  if (n > 0) {
    throw httpError(409, 'Cannot delete group: it still has profiles assigned');
  }
  await db.prepare('DELETE FROM groups WHERE id = ?').run(id);
}
