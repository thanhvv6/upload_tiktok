...
export async function createProfileRecord(db, {
  id, name, group_id, video_folder, channel_ids,
  needs_render, remove_title, need_content_check,
  render_video_long, set_music, render_concat_video,
}) {
  ...
  if (normalizedGroupId) {
    await assertGroupExists(db, normalizedGroupId);
  }
  ...
  try {
    await db.prepare(
      `INSERT INTO profiles
          (id, name, status, is_scheduled, auto_increment_schedule, group_id,
           video_folder, set_music, upload_count, channel_ids,
           needs_render, remove_title, need_content_check,
           render_video_long, render_concat_video)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(
      id,
      trimmedName,
    ...
    if (e && e.code === '23505') {
      ...
      if (msg.includes('profiles_name_key') || msg.includes('profiles_name_unique')) {
